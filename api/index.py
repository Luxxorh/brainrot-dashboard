...
def fetch_live_servers():
    """Fetch live Brainrot servers efficiently (stateless)."""
    try:
        # Step 1: Get Railway API servers
        resp = requests.get(URL, headers=HEADERS, timeout=1)
        data = resp.json()
        if not data:
            return []

        # Step 2: Group jobIds by placeId
        place_map = {}
        name_map = {}
        for server in data:
        ...
        # Step 3: Fetch Roblox API once per placeId
        updated = []
        ...
        # Limit to top 100 servers
        return updated[:100]

    except Exception as e:
        print("Error fetching live servers:", e)
        return []
...
header { background: rgba(255,255,255,0.05); padding: 20px; text-align: center; font-size: 26px; color: #00ffe0; box-shadow: 0 0 12px #00ffe0; backdrop-filter: blur(10px); }
#dashboard { display: flex; flex: 1; overflow: hidden; }
.panel { flex: 1; padding: 20px; overflow-y: auto; border-right: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(8px); }
.panel:last-child { border-right: none; }
h3 { color: #00ffe0; margin-bottom: 12px; }
.entry { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); padding: 12px 16px; margin-bottom: 12px; border-radius: 10px; display: flex; flex-direction: column; animation: fadeIn 0.3s ease-in; transition: transform 0.2s ease; }
.entry:hover { transform: scale(1.02); box-shadow: 0 0 10px #00ffe0; }
.fade-out { animation: fadeOut 0.5s ease forwards; }
.copy-btn { background: #00ffe0; color: #000; border: none; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: background 0.3s ease; margin-right: 6px; }
.copy-btn:hover { background: #00ccaa; color: #fff; }
.meta-block { max-height: 0; opacity: 0; overflow: hidden; transition: max-height 0.4s ease, opacity 0.4s ease; margin-top: 8px; background: rgba(0,0,0,0.3); padding: 8px; border-radius: 6px; white-space: pre-wrap; font-size: 12px; }
.meta-block.show { max-height: 500px; opacity: 1; }
select, input[type="checkbox"] { margin: 6px 0; }
#statBlock { background: rgba(255,255,255,0.1); padding: 16px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); animation: fadeIn 0.6s ease-in-out; }
</style>
</head>
<body>
...
function toggleMeta(jobId) { document.getElementById(`meta-${jobId}`).classList.toggle("show"); }
...
function render(data) {
    ...
    data.forEach(s => {
        ...
        if(div){
            div.querySelector(".server-info").innerHTML=`[${s.timestamp}] <b>${s.name}</b> | ${s.players} → <a href="${s.link}" target="_blank">Join</a>`;
        } else {
